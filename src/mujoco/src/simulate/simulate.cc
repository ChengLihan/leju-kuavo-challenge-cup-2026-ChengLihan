...
protected:
  ...
  void RenderCameraView(int cam_id) {
    ...
    // OpenGL depth buffer 是按"实际米数"做的非线性归一化，linearization 必须用绝对值，
    // 否则得到的距离会少一个 extent 因子（典型 humanoid 场景 extent ~10，导致 0.3m vs 3m 的偏差）。
    const float extent = sim_->m_->stat.extent;
    const float znear = sim_->m_->vis.map.znear * extent;
    const float zfar  = sim_->m_->vis.map.zfar  * extent;
  ...
  void EncodeAndPublishFrame(
    ...
    struct ConfigHeader {
      int format;           // compressionFormat enum
      float depthParam[2];
    } __attribute__((packed));
    ConfigHeader hdr = {};
    hdr.format = 0;  // INV_DEPTH
    hdr.depthParam[0] = 0.f;
