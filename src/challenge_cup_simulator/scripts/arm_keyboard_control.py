...
class LejuClawController:
    def __init__(self):
        ...
        self._state_sub = rospy.Subscriber("/leju_claw_state", lejuClawState, self._state_callback)
        rospy.loginfo("等待 /control_robot_leju_claw 服务...")
        rospy.wait_for_service("/control_robot_leju_claw")
        self._srv = rospy.ServiceProxy("/control_robot_leju_claw", controlLejuClaw)
        rospy.loginfo("乐聚夹爪服务已连接")

    @staticmethod
    def _to_percent(cmd):
        return max(0.0, min(100.0, float(cmd) / 255.0 * 100.0))
    ...
    def _state_callback(self, msg):
        if len(msg.data.position) >= 2:
            self.current_left_cmd = self._clamp_percent(msg.data.position[0])
            self.current_right_cmd = self._clamp_percent(msg.data.position[1])

    def set_gripper_position(self, left_cmd, right_cmd):
        left = self._to_percent(left_cmd)
        right = self._to_percent(right_cmd)
        ...
        try:
            ...
            rospy.logerr("调用 /control_robot_leju_claw 失败: %s", exc)
            return
        if not res.success:
            rospy.logwarn("/control_robot_leju_claw 拒绝命令: %s", res.message)
            return
        self.current_left_cmd = left
        self.current_right_cmd = right

    def control_right_gripper(self, right_cmd):
        self.set_gripper_position(self.current_left_cmd / 100.0 * 255.0, right_cmd)

    def control_left_gripper(self, left_cmd):
        self.set_gripper_position(left_cmd, self.current_right_cmd / 100.0 * 255.0)
...
class KeyBoardArmController:
    def __init__(self, x_gap = 0.03, y_gap = 0.03, z_gap = 0.03, 
        ...
        # 夹爪控制
        self.gripper_controller = LejuClawController()
        # 高度控制
    ...
    def getKey(self):
        ...
        # 手动调整末端姿态
            ...
            try:
                if not self.control_rpy_flag:
                    print("请先按 G 进入手动末端姿态模式，再用 IJKLUO 调整姿态")
                    return ""
