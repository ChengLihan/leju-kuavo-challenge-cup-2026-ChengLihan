...
运行方式：
  rosrun challenge_cup_task_template scene3_tray.py            # 使用默认种子 0
  rosrun challenge_cup_task_template scene3_tray.py --seed 3   # 指定随机种子

注意：--seed 仅影响 scene_builder 构建期的物体随机化；
      scene3 目前 shuffleable_parts 为空，传 seed 基本无效果（仅为接口一致性保留）。
...
def main():
    parser = argparse.ArgumentParser(description="场景三：SMT 料盘出库")
    parser.add_argument("--seed", type=int, default=0,
                        help="随机种子（仅影响构建期物体随机化；scene3 暂无随机化配置，基本无效果）")
    args = parser.parse_args()

    # ---- 启动仿真（生成场景XML + roslaunch + 初始化ROS节点 + 等待就绪） ----
    launcher = ChallengeSimLauncher(scene="scene3", seed=args.seed)
    launcher.start(node_name="scene3_tray")
    ...
    rospy.loginfo("=== 场景三：SMT 料盘出库任务启动 ===")
