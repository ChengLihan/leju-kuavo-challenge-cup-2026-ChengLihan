...
namespace humanoid_controller
{
  ...
  bool humanoidController::init(HybridJointInterface *robot_hw, ros::NodeHandle &controller_nh, bool is_nodelet_node)
  {
    ...
    // If the launch file already provided an initial pose for a scene, keep it.
    if (!init_fall_down_state_) // 初始倒地时不在这里设置初始状态
    {
      if (!ros::param::has("robot_init_state_param"))
        ros::param::set("robot_init_state_param", robot_init_state_param);
    }
  
    if (ros::param::has("robot_init_state_param")) {
      std::vector<double> init_q;
      ros::param::get("robot_init_state_param", init_q);
      ros::param::set("/humanoid/init_q", init_q);
    } else {
